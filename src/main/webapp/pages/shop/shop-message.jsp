...
<html lang="en">
        	...
        	<div class="container text-center">
        		<div class="biao">
        			<center>
										...
										<input type="text" name="sousuo" id="selectvalue"  placeholder="请输入商品名称">
										<button style="border-radius: 5px; border: 1px solid;background: cornflowerblue;color: white" onclick="select()">搜索</button><br />
        				<div>
        				<script src="${pageContext.request.contextPath}/js/jquery.min.js"></script>
									...
									function pageNumber(page,name){
										$.ajax({
										type:'post',
										url:'../../goods.do?method=findbypage',
										data:{"pageNum":page,"gname":name},
										success : function (data){
											...
											for(var i = 0;i<obj.length;i++){
												...
												+"<td>"+obj[i].gnumber+"</td>"
												+"<td>"
												+"<button type='button' onclick='modeldel("+obj[i].gid+")'  class='btn1' id='showModel"+obj[i].gid+"'><a href='#'>删除</a></button>"
												+"<div id='modal"+obj[i].gid+"' class='modal'>"
												+"<div class='modal-content'>"
												+"<header class='modal-header'>"
												+"	<h4>删除信息</h4>"
												+"<span class='close"+obj[i].gid+"'>×</span>"
												+"</header>"
												+"<div class='modal-body'>"
												+"<p>是否要删除？！</p>"
												+"</div>"
												+"<footer class='modal-footer'>"
												+"<button id='cancel"+obj[i].gid+"' class='active'>取消</button>"
												+"<button id='sure'><a href='../../goods.do?method=del&gid="+obj[i].gid+"'>确定</a></button>"
												+"</footer>"
												+"</div>"
												+"</div>"
												+"<button type='button' class='btn2' id='showModel2"+obj[i].gid+"' onclick='modelupdate("+obj[i].gid+")' ><a href='#'>修改</a></button>"
												+"<div id='modal2"+obj[i].gid+"' class='modal'>"
												+"<div class='modal-content2'>"
												+"<header class='modal-header'>"
												+"<h4>修改信息</h4>"
												+"<span class='close2"+obj[i].gid+"'>×</span>"
												+"</header>"
												+"<form action='../../goods.do?method=update' method='post'>"
												+"<div class='modal-body' style='margin-bottom: 50px;'>"
												+"<p><input type='text' name='gid' value='"+obj[i].gid+"' hidden='hidden' /></p>"
												+"<p>商品名字：<input type='text' name='gname' value='"+obj[i].gname+"' /></p>"
												+"<p class='p1'>价格：<input type='text' name='gprice' value='"+obj[i].gprice+"' /></p>"
												+"<p class='p1'>数量：<input type='text' name='gnumber' value='"+obj[i].gnumber+"' /></p>"
												+"</div>"
												+"<footer class='modal-footer'>"
												+"<button id='cancel2"+obj[i].gid+"'>取消</button>"
												+"<button id='sure2' type='submit'>确定</button>"
												+"</footer>"
												+"</form>"
												+"</div>"
												+"</div>"
												+"<button type='button' class='btn3' id='showModel3"+obj[i].gid+"' onclick='model("+obj[i].gid+")' ><a href='#'>增加</a></button>"
												+"<div id='modal3"+obj[i].gid+"' class='modal'>"
												+"<div class='modal-content3'>"
												+"<header class='modal-header'>"
												+"<h4>增加商品</h4>"
												+"<span class='close3"+obj[i].gid+"'>×</span>"
												+"</header>"
												+"<form action='../../goods.do?method=add' method='post'>"
												+"<div class='modal-body' style='margin-bottom: 50px;'>"
									            +"<p>商品名字：<input type='text' name='gname' value='' /></p>"
									            +"<p class='p1'>价格：<input type='text' name='gprice' value='' /></p>"
									            +"<p class='p1'>数量：<input type='text' name='gnumber' value='' /></p>"
									            +"</div>"
									            +"<footer class='modal-footer'>"
									            +"<button id='cancel3"+obj[i].gid+"' type=\"button\">取消</button>"
									            +"<button id='sure3' type='submit'>确定</button>"
									            +"</footer>"
									            +"</form>"
									            +"</div>"
									            +"</div>"
	...
	function model(id){
		var btn3 = document.getElementById('showModel3'+id+'');
		var close3 = document.getElementsByClassName('close3'+id+'')[0];
		var cancel3 = document.getElementById('cancel3'+id+'');
		var modal3 = document.getElementById('modal3'+id+'');
