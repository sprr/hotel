							...
							<div class="biao">
								<center>
									<form action="" method="post" class="sousuo">
										<input type="text" id="selectvalue" name="uname" placeholder="请输入用户名"><button type="button" onclick="select()">搜索</button>
									</form><br />
									<div>
				                    ...
				                    function getcount(content) {
				      				...
				      				function pageNumber(page,content) {
										$.post({
											type:'post',
											url:'../../blog.do?method=comment',
											data:{"pageNum":page,"uname":uname},
											success : function(data) {
												var obj = JSON.parse(data);
												var str = "<tr class='tr'>"
												+"<td>评论编号</td>"
												+"<td>用户姓名</td>"
												+"<td style='width:520px'>评论内容</td>"
												+"<td>时间</td>"
												+"<td>操作</td>"
												+"</tr>";
												for (var i = 0; i < obj.length; i++) {
													str+="<tr>"
														+"<td>"+obj[i].coid+"</td>"
														+"<td>"+obj[i].uname+"</td>"
														+"<td style='width:520px'>"+obj[i].message+"</td>"
														+"<td>"+obj[i].cotime+"</td>"
														+"<td>"
														+"<button type='button' onclick='modeldel("+obj[i].coid+")' class='btn1' id='showModel"+obj[i].coid+"'><a>删除</a></button>"
														+"<div id='modal"+obj[i].coid+"' class='modal'>"
														+"<div class='modal-content'>"
														+"<header class='modal-header'>"
														+"<h4>删除信息</h4>"
														+"<span class='close"+obj[i].coid+"'>×</span>"
														+"</header>"
														+"<div class='modal-body'>"
														+"<p>是否要删除？！</p>"
														+"</div>"
														+"<footer class='modal-footer'>"
														+"<button type='button' id='cancel"+obj[i].coid+"' class='active'>取消</button>"
														+"<button id='sure'><a href='../../blog.do?method=delcomment&coid="+obj[i].coid+"'>确定</a></button>"
														+"</footer>"
														+"</div>"
														+"</div>"
														+"</td>"
									...
									function modeldel(id) {
										...
										<table id="commentall" class="biaoge" border="1px solid black" ; cellpadding="0" cellspacing="0">
